...
- **Vendor helpdesk (`NyxiteSupport`) — separate, isolated infra, NOT part of this stack.** Like `NyxiteLicense`, the central bug-reporting / ticketing helpdesk is run by the maintainer as isolated vendor infra (its own host/container, no path to any content plane); self-hosters never run it. The self-hosted server only **relays** authenticated reports to it, and only when reporting is enabled for the instance (`support.enabled`; v1 = the maintainer's official instance(s) — SUP-9). It runs on the consensual non-E2EE support plane, disjoint from content. Server-side relay config: `NYXITE__Support__Enabled`, `__ServiceBaseUrl`, `__InstanceCredential`. See [features/support.md](support.md).
